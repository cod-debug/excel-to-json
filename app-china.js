...
const excelFilePath = 'New Colleague Data File_GC_07November.xlsx';
...
brand_list.map((i) => {
    ...
    for (const role in i.role_headers){
        china.china[i.brand_id]['departments'][role] = { name: capitalizeEachWord(role.replaceAll("-", " "))}
    }
});

// Alternatively, you can write the extracted data to a new JSON file
const jsonFilePath = `brand-departments-china.json`;
        ...
        */
        ...
        for (const role_id in item.role_headers) {
            ...
            if (item.role_headers.hasOwnProperty(role_id)) {
                ...
                // Alternatively, you can write the extracted data to a new JSON file example output "/china/china.regent.general-manager.json"
